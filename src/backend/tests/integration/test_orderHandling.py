...
from router.orderItem import CreateOrderRequest, OrderItemRequest
...
async def test_order_menu_items_valid(
    ...
    response = await customer_tablet_client.post("/order", json=payload.model_dump())

    assert response.status_code == 200
    status_update_payload = {"status": serialize_order_status(OrderStatus.PREPARING)}
    ...
    update_response = await manager_client.post(url, json=status_update_payload)
