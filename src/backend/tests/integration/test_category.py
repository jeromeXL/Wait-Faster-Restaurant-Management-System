...
async def test_create_category_valid(manager_client):
    ...
    # Create a new menu item
    response = await manager_client.post(
        "/category/", json={"name": "Test Item", "menu_items": []}
    )
    assert response.status_code == 200
...
async def test_update_category_invalid_does_not_exist(manager_client):
    ...
    response = await manager_client.put(
        "/category/65f22e2c47277444c60898a6",
        json={"name": "notexist", "menu_items": []},
    )
...
async def test_update_category_category_id_invalid_format(manager_client):
    ...
    # attempt to update any category
    response = await manager_client.put(
        "/category/notreal", json={"name": "notexist", "menu_items": []}
    )
    assert response.status_code == 400
...
async def test_update_category_valid(manager_client):
    ...
    response = await manager_client.put(
        f"/category/{category['id']}", json={"name": "Change", "menu_items": []}
    )
...
async def test_delete_category_valid(manager_client):
    ...
    response = await manager_client.post(
        "/category/", json={"name": "del1", "menu_items": []}
    )
    category1 = response.json()

    response = await manager_client.post(
        "/category/", json={"name": "del2", "menu_items": []}
    )
    category2 = response.json()

    response = await manager_client.post(
        "/category/", json={"name": "del3", "menu_items": []}
    )
    ...
    categoryWithId = await Category.find_one(
        Category.id == PydanticObjectId(category2["id"])
    )
