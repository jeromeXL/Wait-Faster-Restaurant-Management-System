...
async def test_active_session_already_exists(admin_client: AsyncClient):
    ...
    create_session_response2 = await admin_client.post(
        "/session/start", headers={"Authorization": f"Bearer {tokens['access_token']}"}
    )
...
async def test_session_complete(admin_client: AsyncClient):
    ...
    login_response_ct = await admin_client.post(
        "/auth/login", json={"username": "Table1", "password": "Table1"}
    )
    ...
    create_session_response = await admin_client.post(
        "/session/start",
        headers={"Authorization": f"Bearer {tokens_ct['access_token']}"},
    )
    assert create_session_response.status_code == 200
    ...
    login_response_ws = await admin_client.post(
